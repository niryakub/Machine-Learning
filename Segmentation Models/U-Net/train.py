...
def train_fn(loader, model, optimizer, loss_fn, scaler):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        # Forward
        with torch.cuda.amp.autocast(): # autocast enables faster computation upon forward&loss, it uses less accurate dtypes yet they require less ram
...
def main():
    ...
    train_transform = A.Compose(
        [
            ...
            A.Normalize(
                mean=[0.0, 0.0, 0.0],
                std=[1.0, 1.0, 1.0],
                max_pixel_value=255.0, # this divides pixels by 255
            ),
            ToTensorV2(),
        ],
    )
    ...
    loss_fn = nn.BCEWithLogitsLoss() # performs both sigmoid layer & BCE over our output layer
